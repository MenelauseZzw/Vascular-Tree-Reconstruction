...
void testKnnSearch(float* pP, float *pSigma, float* pS, float* pT, std::vector<int>& indPi, std::vector<int>& indPj, int numPoints)
{
  ...
  for (int i = 0; i < query.rows; ++i)
  {
    for (int k = 0; k < nn; ++k)
    {
      ...
      if (i < j)
      {
        ...
        if (2 * distanceBetweenLines < pSigma[i] + pSigma[j])
        {
          indPi.push_back(i);
          indPj.push_back(j);
        }
