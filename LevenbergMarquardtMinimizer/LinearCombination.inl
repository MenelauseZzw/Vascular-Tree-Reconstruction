...
struct ColumnIndices2
{
	...
	__host__ __device__ IndexType operator()(IndexType i) const
	{
		...
		index = k % NumDimensions + NumDimensions * (k / NumDimensions2);// the storage of variable is (s..s,t..t) not like that in Egor's thesis
...
void LinearCombination<NumDimensions, ValueType, IndexType, MemorySpace>::ComputeJacobian(const VariableVectorType& x, JacobianVectorType& jacobian) const
{
	...
	//const auto t = x.subarray(s.size(), x.size());
	...
	ComputeJacobian2(
		...
		thrust::raw_pointer_cast(&weights3[0]),
...
//CPU version can only deal with non-oriented curvature regularization 
template<int NumDimensions, typename ValueType, typename IndexType>
void CpuLinearCombination<NumDimensions, ValueType, IndexType>::ComputeResidual2(const ValueType* pTildeP, const ValueType* pS, const ValueType* pT, const ValueType* pWeights, const ValueType* pWeights3, const IndexType* pIndices1, const IndexType* pIndices2, ValueType* pResidual, int residualVectorLength, double tau) const
{
	BOOST_LOG_TRIVIAL(info) << "Computing residuals";
...
//CPU version can only deal with non-oriented curvature regularization 
template<int NumDimensions, typename ValueType, typename IndexType>
void CpuLinearCombination<NumDimensions, ValueType, IndexType>::ComputeJacobian2(const ValueType* pTildeP, const ValueType* pS, const ValueType* pT, const ValueType* pWeights, const ValueType* pWeights3, const IndexType* pIndices1, const IndexType* pIndices2, ValueType* pJacobian, int residualVectorLength, double tau) const
{
	BOOST_LOG_TRIVIAL(info) << "Computing Jacobian matrix";
...
//GPU version can deal with both oriented curvature and divergence terms
template<int NumDimensions, typename ValueType, typename IndexType>
void GpuLinearCombination<NumDimensions, ValueType, IndexType>::ComputeResidual2(const ValueType* pTildeP, const ValueType* pS, const ValueType* pT, const ValueType* pWeights, const ValueType* pWeights3, const IndexType* pIndices1, const IndexType* pIndices2, ValueType* pResidual, int residualVectorLength, double tau) const
{
	BOOST_LOG_TRIVIAL(info) << "Computing residuals";
	...
	gpuPairwiseCostResidual<ValueType, IndexType, NumDimensions>(pTildeP, pS, pT, pWeights, pWeights3, pIndices1, pIndices2, pResidual, residualVectorLength, tau);
}
...
void GpuLinearCombination<NumDimensions, ValueType, IndexType>::ComputeJacobian2(const ValueType* pTildeP, const ValueType* pS, const ValueType* pT, const ValueType* pWeights, const ValueType* pWeights3, const IndexType* pIndices1, const IndexType* pIndices2, ValueType* pJacobian, int residualVectorLength, double tau) const
{
	...
	gpuPairwiseCostJacobian<ValueType, IndexType, NumDimensions>(pTildeP, pS, pT, pWeights, pWeights3, pIndices1, pIndices2, pJacobian, residualVectorLength, tau);
}
